...
def build_doc():
    ...
    add_para(doc, "当前发货单仍绑定原始订单，因此还需补齐退款、取消与退货场景：售后业务归属于原订单商品行，物流履约归属于实际发货单商品行，系统通过数量来源映射同时保留两种关系，避免拆合后退货无法定位或错误冲减其他原订单。")
    ...
    add_bullets(doc, [
        ...
        ("售后闭环：", "退货仍从原订单商品行发起，并能定位拆合后的实际发货单及可退数量；发货前退款或取消实时扣减可履约数量。"),
    ...
    add_table(
        ...
        [
            ...
            ["退货关联与售后联动", "复用现有售后退款模块：售后归属原订单商品行，同时关联实际发货单商品行；拆合前校验退款、取消、退货占用数量。", "发货管理入口 + 售后退款流程"],
        ],
        [1600, 5100, 2660],
        font_size=8.8,
    )
    ...
    add_table(
        ...
        [
            ...
            ["发货前发生退款或取消", "仅对扣除售后占用后的剩余履约数量拆合", "发货管理 / 旺店通驳回订单Tab", "全量取消时当前发货单失效，不再拆合"],
            ["发货后发生退货", "从原订单商品行发起退货，并映射实际发货单明细", "售后退款模块 / 发货管理退货关联", "原订单承担售后归属，实际发货单承担退货履约追溯"],
        ],
        [2700, 2200, 2000, 2460],
        font_size=8.7,
    )
    ...
    add_table(
        ...
        [
            ...
            ["P0", "退货关联与售后数量校验", "保存原订单商品行与实际发货单商品行映射；拆合提交时重新校验退款、取消、退货占用及剩余可履约数量。"],
    ...
    add_heading(doc, "6.3 退货业务关系图", 2)
    add_para(doc, "本图仅用于PRD评审时说明退货与原订单、实际发货单的业务关系，不作为系统内退款关联页面的展示组件。系统页面直接展示数量摘要和关联明细。", color=MUTED, size=9.2, after=5)
    ...
    add_table(
        ...
        [
            ...
            ["退货关联弹窗", "从发货单、驳回订单或拆合记录进入；分别展示发货前退款/取消和发货后退货。页面不展示流程图，直接使用数量摘要卡片和关联明细表呈现原订单商品行、实际发货单商品行、已退数量及可退数量。"],
        ],
        [2500, 6860],
        font_size=9,
    )
    ...
    add_table(
        ...
        [
            ...
            ["发货前退款/取消", "退款或取消成功后先扣减剩余履约数量，拆单、合单与同步旺店通只处理剩余数量；某发货单剩余数量为0时，该发货单失效且不可继续拆合或同步。"],
            ["拆单后退货", "一个原订单商品行可能分布在多个拆单子发货单。退货从原订单商品行发起后，应按实际发货明细选择或分配到一个或多个子发货单。"],
            ["合单后退货", "一个合并发货单可能包含多个原订单。退货必须先确定原订单商品行，再定位合并发货单中的来源数量；不得以合并发货单为共同归属冲减其他原订单。"],
            ...
            ["部分退货", "一笔售后可关联多个实际发货单商品行，映射数量之和必须等于本次退货数量；允许拆单子单分别退回，但仍归属于同一原订单售后。"],
            ["驳回订单处理", "旺店通驳回订单提交拆合前必须重新读取最新售后状态和剩余履约数量；全量退款或取消时不再拆合，按取消结果结束当前发货单。"],
            ["并发与幂等", "拆合提交和退货创建均进行版本及数量复核；重复请求不得重复占用数量，累计退货数量不得超过累计已发货数量。"],
            ["历史保留", "退货完成不回写或覆盖既有拆合记录；关联单据链需同时展示原订单、拆合批次、实际发货单及退货单。"],
            ["页面展示", "系统内退款/退货关联页面不展示流程图，直接展示订单与数量摘要、原订单商品行、实际发货单商品行、已发数量、已退数量和可退数量。"],
        ],
        [2200, 7160],
        font_size=8.7,
    )
    ...
    add_bullets(doc, [
        ...
        "售后人员仍从原订单发起退款或退货；页面新增实际发货单关联信息，不改变现有售后归属和财务口径。",
    ])
    ...
    add_table(
        ...
        [
            ...
            ["售后/退款/退货", "仍以原订单商品行为业务归属；新增原订单商品行—实际发货单商品行—退货明细的数量映射，并向拆合服务提供售后占用及剩余履约数量。"],
    ...
    add_table(
        ...
        [
            ["旺店通接口负责人", "状态查询与订单同步接口", "确认可查询状态字段、调用频率、幂等字段、旺店通单号返回时点。", "P0"],
            ...
            ["订单/售后系统", "退款、取消、退货状态与数量联动", "提供原订单商品行的售后占用、累计有效退货和剩余可履约数量；保存售后明细到实际发货单商品行的数量映射。", "P0"],
            ["业务运营", "合单准入与操作流程", "确认客户、收件人、手机号、地址必须一致；完成UAT场景验收。", "P0"],
            ["测试团队", "端到端与并发测试", "覆盖前置拆合、驳回拆合、多级拆合、重复提交、部分同步、再次驳回。", "P0"],
    ...
    add_bullets(doc, [
        ...
        "订单/售后系统的数量占用状态枚举、释放时点，以及退款、取消、拒收是否共用同一可退数量口径。",
        "一笔售后跨多个实际发货单时，退货单由售后系统统一生成还是按实际发货单拆分生成。",
