...
def make_process_diagram(path):
    ...
    d.text((60, 92), "发货管理使用双Tab承载不同状态的订单；驳回后可直接重新同步，也可拆合后同步新结果单。", fill="#5D6B78", font=small)
    ...
    draw_rounded_box(d, (625, 500, 905, 630), "#FFF3E8", "#E8A55A", "选择处理方式", "直接重同步 / 拆单 / 合单")
    draw_rounded_box(d, (1015, 500, 1295, 630), "#EAF6EC", "#77B980", "确定同步对象", "原发货单 / 新结果发货单")
    draw_rounded_box(d, (1405, 500, 1690, 630), "#EAF4FD", "#77B5E8", "标记已处理", "提交旺店通并等待查询")
    ...
    d.text((962, 762), "若重新同步或拆合后的结果再次被驳回：新增驳回记录并关联上次处理信息，不覆盖历史链路。", fill="#4D5C6A", font=small, anchor="mm")
    img.save(path)
...
def build_doc():
    ...
    add_callout(
        doc,
        "核心产品定义",
        "系统以“当前有效发货单”为操作对象：发货管理统一设置“发货订单 / 旺店通驳回订单”两个Tab。未同步旺店通的发货单可直接拆合；查询到“已驳回”后，可原单重新同步，也可拆单或合单后同步新结果单。每次处理均保留请求与单据关系，历史永久保留。",
        "info",
    )
    ...
    add_para(doc, "同时，旺店通目前不会主动推送驳回详情。OMS只能通过接口主动查询订单状态，并且仅能获知“已驳回”，无法获得驳回原因、错误类型或处理建议。因此，需要由业务人员结合订单信息，在OMS内选择直接重新同步、拆单或合单。")
    ...
    add_bullets(doc, [
        ...
        ("驳回重试：", "商品与数量无需调整时，可直接重新同步当前有效发货单，不强制先拆单或合单。"),
        ...
        ("简单闭环：", "驳回订单仅区分未处理与已处理；直接重新同步的请求结果在原驳回记录查看，拆单和合单结果进入“拆/合单管理”。"),
    ])
    ...
    add_table(
        ...
        ["模块", "功能范围", "页面定位"],
        [
            ["发货管理", "统一设置“发货订单 / 旺店通驳回订单”Tab。发货订单Tab支持未同步旺店通的有效发货单直接拆合；旺店通驳回订单Tab提供重新同步、拆单和合单三种处理方式，完成任一处理后标记已处理。", "统一操作页面 + 双Tab"],
            ...
            ["售后订单管理", "新增售后订单列表与申请售后页面。支持仅退款、退货退款；申请时选择退款原因，按实际发货明细填写数量和金额，提交后生成处理中售后单。售后完成后向旺店通推送1条数据，并关联原订单商品行与实际发货单商品行。", "售后退款菜单 + 发货管理关联入口"],
        ],
        [1600, 5100, 2660],
        font_size=8.8,
    )
    ...
    add_table(
        ...
        [
            ...
            ["旺店通查询为已驳回", "直接重新同步、拆单、合单", "发货管理 > 旺店通驳回订单Tab", "重新同步原单，或生成结果单后同步；本次驳回标记已处理"],
            ["重新同步或拆合结果再次被驳回", "再次重新同步、拆单或合单", "发货管理 > 旺店通驳回订单Tab", "新增驳回记录并关联上次处理，不覆盖历史"],
    ...
    add_table(
        ...
        [
            ...
            ["P0", "驳回订单处理", "在旺店通驳回订单Tab主动查询状态；未处理记录支持直接重新同步、拆单或合单。重新同步不生成新发货单；任一处理提交成功后，本次驳回记录标记已处理。"],
            ...
            ["P0", "申请售后与售后订单管理", "提供紧凑型申请售后弹窗和售后订单列表；支持仅退款、退货退款、数量/金额校验、详情查看与完成操作。保存原订单商品行与实际发货单商品行映射；售后完成后按售后订单维度向旺店通幂等推送1条数据。"],
    ...
    doc.inline_shapes[-1]._inline.docPr.set("descr", "展示发货管理内发货订单与旺店通驳回订单两个Tab的处理流程；驳回订单可直接重新同步，也可拆合后同步新结果单。")
    add_caption(doc, "图1  发货管理双Tab与驳回订单三种处理方式")
    ...
    add_table(
        ...
        ["原型页面", "核心交互"],
        [
            ["发货管理", "同一页面使用“发货订单 / 旺店通驳回订单”Tab切换。发货订单Tab对未同步单据显示拆单、合单；旺店通驳回订单Tab可主动刷新状态，对未处理记录显示重新同步旺店通、拆单、合单三个操作。直接重新同步后仅显示已处理并保留查看订单入口，不新增同步记录页面；拆合后进入对应结果档案。"],
            ...
            ["售后订单页面", "在售后退款菜单下新增页面；采用指标卡、紧凑筛选区和列表。列表展示售后订单/原订单、售后类型、商品与申请数量、客户、退款金额、售后状态、旺店通推送状态与条数、申请时间，并提供查看详情、完成售后、查看推送操作。"],
            ["申请售后弹窗", "参考售后关联页的紧凑视觉：顶部切换仅退款/退货退款，依次展示说明条、商品摘要、数量指标卡、实际发货明细表、退款原因与金额表单、售后描述及凭证上传。提交后生成处理中售后单，不在申请阶段推送旺店通。"],
    ...
    add_bullets(doc, [
        ...
        "未处理的驳回订单提供直接重新同步、拆单和合单三个操作。重新同步沿用当前有效发货单及原商品数量，不生成新发货单。",
        "不加锁，采用乐观校验：任一处理提交前，再次校验该驳回订单仍为未处理、当前发货单仍有效，且售后占用未导致履约数量变化；校验失败时终止提交并保留页面输入。",
        "重新同步请求被OMS接口受理，或拆合结果成功生成后，来源驳回记录标记已处理，并保存处理类型、同步请求号或拆合结果编号。",
        "重新同步提交成功只表示请求已发送，不代表旺店通已接收。OMS继续主动查询；查询为已接收时进入正常发货，查询为已驳回时新增驳回记录。",
        "页面不新增同步记录功能。重新同步的处理类型和请求号保存在原驳回订单详情及接口日志中；用户仍通过主动查询刷新旺店通状态。",
        "重新同步或拆合结果再次被驳回时，新增记录并关联上一次驳回记录、处理类型和请求号；不得覆盖历史。",
    ])
    ...
    add_table(
        ...
        [
            ...
            ["驳回后重新同步", "重新同步使用新的同步请求号，但OMS发货单ID保持不变；同一次按钮提交或网络重试复用同一请求号，不得重复创建旺店通订单。"],
    ...
    add_table(
        ...
        [
            ...
            ["售后类型", "申请售后支持“仅退款”和“退货退款”。切换类型时保留当前订单与商品上下文，并根据类型更新数量字段名称和提示文案。"],
            ["申请数量", "按实际发货单商品行填写正整数；本次申请数量不得超过该原订单商品行在对应实际发货明细上的可申请数量。多条明细的申请数量合计构成本次售后数量。"],
            ["退款原因", "退款原因为必填项；未选择时不得提交。售后相关描述和图片凭证本期为选填，业务另有强制要求时再配置。"],
            ["退款金额", "申请退款金额必须大于0且不得超过按本次申请数量计算的可退款金额；金额保留两位小数。"],
            ["退款构成", "进场抵扣退款、上账退款、实际支付退款均不得小于0，三项合计必须等于本次申请退款金额。"],
            ["申请提交", "提交成功后生成唯一售后订单并进入处理中；重复点击或网络重试返回同一结果。申请创建、审核或处理中均不向旺店通推送。"],
    ...
    add_table(
        ...
        [
            ...
            ["重新同步前订单已由他人处理", "提交时乐观校验未通过，终止本次请求并刷新记录。", "订单已由其他人员处理，请查看结果。"],
            ["重新同步前发货单已失效或售后数量变化", "不发送旺店通请求，保留当前页面并刷新可履约数量。", "订单状态或可履约数量已变化，请刷新后重新选择处理方式。"],
            ["重新同步接口提交失败", "本次驳回记录保持未处理，保存失败原因并允许使用同一请求号重试。", "重新同步失败，请重试；商品和数量未发生变化。"],
            ["重新同步请求重复提交", "按OMS发货单ID和本次业务请求号进行幂等校验，返回首次提交结果。", "已提交重新同步，请勿重复操作。"],
            ["重新同步后再次被驳回", "新增驳回记录并关联上一次驳回记录和同步请求号。", "订单再次被旺店通驳回，可重新同步、拆单或合单。"],
            ...
            ["申请售后未选择退款原因", "阻止提交并定位到退款原因字段。", "请选择退款原因。"],
            ["申请数量为空、非正整数或超过可申请数量", "阻止提交并展示当前可申请上限。", "申请数量需为正整数且不能超过可申请数量。"],
            ["申请退款金额为0或超过可退款金额", "阻止提交，保留已填表单。", "申请退款金额需大于0且不能超过可退款金额。"],
            ["退款构成合计与申请金额不一致", "阻止提交并高亮进场抵扣、上账、实际支付三个金额字段。", "退款构成合计必须等于申请退款金额。"],
            ["重复点击申请售后确认", "以申请请求号和业务唯一约束返回首次创建结果。", "售后订单已创建，请勿重复提交。"],
    ...
    add_bullets(doc, [
        ...
        "发货管理统一为“发货订单 / 旺店通驳回订单”双Tab：前者承担未同步订单的拆合，后者提供直接重新同步、拆单和合单三种驳回处理方式。",
        "原拆单管理、合单管理合并为“拆/合单管理”，通过Tab查看两类完成结果；培训需说明拆单按处理前→处理后阅读，合单按处理后→处理前阅读。",
        "已接收旺店通的订单必须保持不可拆合，避免第三方重复发货。",
        "售后人员可在订单商品行发起申请，并在新增售后订单页面统一查询和处理。提交申请只生成处理中售后单；完成售后后系统才自动向旺店通推送1条数据。",
    ])
    ...
    add_table(
        ...
        [
            ...
            ["旺店通同步", "同步对象从原订单固定映射改为当前有效发货单；驳回后可用新的同步请求号重新提交同一发货单，并保存历次请求及查询结果。"],
            ["状态查询任务", "定时查询未完成发货单状态；识别已驳回并写入驳回订单。"],
            ["发货管理页面", "驳回订单操作区增加重新同步旺店通；确认弹窗说明不生成新发货单及后续查询规则。处理后沿用原订单详情，不增加同步记录页面。"],
            ["售后/退款/退货", "以售后订单为发起与推送主体；保留原订单商品行—实际发货单商品行的数量映射，售后完成后按售后订单维度向旺店通推送1条数据。"],
            ["售后订单页面", "新增售后订单列表、紧凑筛选区、指标卡、申请售后弹窗、详情与推送结果展示；申请表单增加数量上限、可退金额及退款构成一致性校验。"],
    ...
    add_numbered(doc, [
        ...
        "开放同页旺店通驳回订单Tab的直接重新同步、拆单和合单，并分别监控原单重试与新结果单的接收率、再次驳回率。",
    ...
    add_table(
        ...
        [
            ["旺店通接口负责人", "订单状态查询、发货同步及售后推送接口", "确认已驳回订单能否以同一OMS发货单ID和新的请求号重新提交，以及幂等、结果查询、失败重试和售后推送能力。", "P0"],
            ["OMS后端", "发货单处理与关系模型", "实现驳回原单重新同步、请求幂等、处理结果记录，以及拆合事务、当前有效标识、商品来源映射和接口日志。", "P0"],
            ["OMS前端", "发货管理、驳回重同步、拆合弹窗、结果档案及售后页面", "在驳回订单行和详情中增加重新同步入口、确认弹窗、提交反馈与结果查看；同时完成拆合及售后页面交互。", "P0"],
            ["商品标准库/主数据", "商品温层字段", "为参与发货的SKU维护常温、冷藏、冷冻枚举值，并确认接口取值、历史数据补齐和变更生效时点。", "P0"],
            ["订单/售后系统", "售后申请、数量金额校验、状态与完成事件", "提供仅退款/退货退款创建、退款原因、数量上限、可退金额和退款构成校验；保存原订单商品行到实际发货单商品行的数量映射，并保证一张售后订单完成只触发1条旺店通数据。", "P0"],
            ["业务运营", "合单准入与操作流程", "确认客户、收件人、手机号、地址必须一致；完成UAT场景验收。", "P0"],
            ["测试团队", "端到端、表单校验与并发测试", "覆盖驳回后直接重同步、提交失败、重复点击、并发处理、再次驳回，以及前置拆合、多级拆合和售后推送。", "P0"],
    ...
    add_bullets(doc, [
        ...
        "已驳回订单直接重新同步时，旺店通是否允许沿用原业务单号，以及新请求与原旺店通单号的关联和去重规则。",
    ...
    add_callout(
        doc,
        "评审结论建议",
        "本期按P0范围建设“OMS当前有效发货单处理”能力。旺店通驳回订单支持直接重新同步、拆单和合单；拆/合单管理继续作为重组结果档案。上线前必须确认旺店通重同步规则，并完成状态准入、请求幂等、再次驳回和端到端映射验收。",
        "success",
    )
