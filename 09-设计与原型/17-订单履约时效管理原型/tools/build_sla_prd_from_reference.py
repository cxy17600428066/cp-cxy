...
box(d,1220,175,720,160,'两项完成后同步旺店通',['接单不等待财务通过','接单即开始发货计时'],'gray')
txt(d,60,490,'后续按各自业务事件计时',34,bold=True,anchor='lm')
items=[('发货',['工厂接单成功起算','WMS收单且生成单号结束'],'blue'),('揽收',['WMS收到ERP订单起算','WMS出库完成结束'],'orange'),('签收',['WMS出库完成起算','物流签收或回执确认'],'blue'),('回执上传',['签收统计时间起算','首次成功上传结束'],'green')]
...
pill(d,800,'无需回执则跳过上传时效；回执审核单独记录，驳回和补传不改变首次上传时间。')
...
box(d,1370,300,570,160,'接单选择有货或无货',['商品时效用于发货节点'],'orange')
